...
### تحقق النشرة `dpl_H4yrbLJwr6PqXyQgvRjJL13S84zY`

أصبحت النشرة المرتبطة بالالتزام `1ff0f99f` في حالة `READY` على رابط Vercel الخاص بها. سُجِّل الدخول بنجاح بحساب QA الحالي في هذه النشرة، وظهر سجل الدين `DBT-2026-0001`، ما يؤكد سلامة المصادقة والوصول إلى بيانات الاختبار بعد نشر إصلاح المعاينة.

أظهر فحص هذا الحساب أن سجل `DBT-2026-0001` يحوي مرفق PNG مضمنًا من اختبار أقدم، وأن عداد المرفقات المنفصلة يساوي صفراً؛ لذلك ليس هو سجل WebP الذي رُفع في جولة Firebase السابقة. لتجنب إنشاء حساب إضافي، سيُرفع ملف WebP غير حساس جديد إلى سجل QA المصادق الحالي نفسه، ثم تُختبر المعاينة المصادَق عليها والمراجعة وسجل التدقيق، وبعدها تُحذف المرفقات والبيانات التجريبية المضافة.

نجح رفع ملف الاختبار غير الحساس `qa-protected-preview.png` إلى سجل QA الحالي على النشرة `dpl_H4yrbLJwr6PqXyQgvRjJL13S84zY`. ظهر في القائمة كمرفق واحد بحالة **قيد المراجعة** مع زر معاينة، وزر فتح مستقل، وزر مراجعة، وزر ملاحظة داخلية، وسجل تدقيق. يؤكد ذلك عمل الرفع المصادق وقاعدة Firebase المقيدة مع حساب QA الحالي.

**تشخيص المعاينة بعد النشر:** النقر على الملف يستدعي مسار التطبيق المحمي بنجاح، لكن تنزيل المتصفح المباشر من Firebase عبر ترويسة المصادقة يفشل برسالة `Failed to fetch`. هذا ناتج عن طلب CORS preflight من نطاق Vercel إلى Firebase Storage؛ لا يدل على فشل الملكية أو القاعدة. سيُنقل تنزيل ملف Firebase إلى مسار التطبيق المحمي في الخادم، حيث تُمرَّر هوية المستخدم إلى Firebase من دون CORS، ثم يُعاد محتوى الملف مباشرة للمتصفح. بذلك تظل الحاوية مقيدة ولا يحتاج المنتج إلى فتح CORS أو مشاركة رابط تنزيل عام.

### إصلاح تنزيل المعاينة عبر الخادم — جاهز للنشر

يعيد مسار المعاينة الآن، لملفات Firebase فقط، رابطاً داخلياً يتضمن `download=1` بدلاً من رابط Firebase المباشر. عند طلب هذا الرابط، يتحقق الخادم من هوية المستخدم وملكية السجل والمرفق مرة ثانية، ثم ينزّل المحتوى من Firebase برمز ID Token ذاته ويعيده باستجابة خاصة غير قابلة للتخزين المؤقت (`private, no-store`). يحصل العميل على Blob من نفس نطاق التطبيق، فيتجنب CORS من دون تخفيف قاعدة Firebase أو كشف رابط تنزيل عام.

اجتازت النسخة المحلية **65 اختبار وحدة**، وفحص TypeScript وبناء الإنتاج كاملاً. يلزم نشر هذا الإصلاح وإعادة اختبار المعاينة والمراجعة وسجل التدقيق في Vercel.
