...
### تحقق إصلاح تنزيل المعاينة عبر الخادم في الإنتاج (17 أغسطس 2026)

| البند | النتيجة |
|---|---|
| النشرة المختبرة | نشرة الإنتاج الجديدة المرتبطة بنقطة الإصدار `b594e2cc` عبر رابط Vercel محمي ومصرح مؤقتاً |
| المصادقة والوصول | نجح الدخول بحساب QA V2 وفتح سجل الدين `DBT-2026-0001` ولوحة «فواتير وإيصالات» |
| معاينة المرفق | نجحت معاينة `qa-protected-preview.png`؛ استلمت الواجهة ملفاً من المسار الداخلي المحمي وعرضته عبر `Blob URL` محلي دون خطأ CORS أو `Failed to fetch` |
| مراجعة المرفق | نجح تغيير الحالة من «قيد المراجعة» إلى «تمت المراجعة»؛ أكدت الواجهة الرسالة: «تم تعليم «qa-protected-preview.png» كمراجع وتوثيق العملية.» |
| الأمان | لم تُخفف قواعد Firebase Storage ولم يُكشف رابط تنزيل عام؛ بقي التنزيل محكوماً بالتحقق من الجلسة وملكية السجل والمرفق |

بقي التحقق البصري النهائي من ظهور حدث سجل التدقيق داخل الواجهة، ثم حذف مرفق الاختبار وسجل الدين وحساب QA وفق خطوات التنظيف المعتمدة.

### تحقق سجل التدقيق

أظهرت بطاقة المرفق بعد تغيير الحالة عداد **«سجل التدقيق (1)»**، كما ظهر النص المرتبط بالحدث: **«بواسطة اختبار إنتاجي V2 · qa.production.v2.20260816215034@example.com»**. يثبت ذلك أن حدث المراجعة حُفظ مع هوية المراجع، إضافةً إلى رسالة الواجهة التي تؤكد توثيق العملية. وبذلك اكتمل اختبار الرفع والمعاينة والمراجعة وسجل التدقيق على النشرة الجديدة.

### تنظيف بيانات QA — حالة التنفيذ

حُذف مرجع `qa-protected-preview.png` من سجل الدين عبر واجهة التطبيق، ثم حُذف سجل الدين التجريبي `DBT-2026-0001` بنجاح؛ عادت الواجهة لتعرض «جميع الديون (0)». بعد ذلك استُخدم رمز جلسة QA المصادق عليه لحصر كائنات Firebase Storage تحت المسار المملوك للحساب، فتم العثور على **كائن واحد** وحذفه بنجاح.

محاولة حذف حساب Firebase Auth الخاص بـ QA عبر واجهة الحسابات انتهت باستجابة `400` دون تنفيذ الحذف. لذلك تظل بيانات التطبيق وكائن التخزين منظفة، بينما حذف حساب المصادقة نفسه قيد التشخيص قبل اعتباره مكتملاً؛ لا ينبغي إعادة محاولة الطلب نفسه من دون استخراج رمز الخطأ المحدد أو تجديد رمز الجلسة.

أظهر التشخيص أن الرمز المميز ما زال صالحاً زمنياً، لكن Firebase أعاد الخطأ `CREDENTIAL_TOO_OLD_LOGIN_AGAIN`. ويعني ذلك أن حذف الحساب عملية حساسة تتطلب إعادة تسجيل دخول حديثة؛ سيتم تجديد جلسة QA ثم تنفيذ طلب الحذف المصرح به مرة واحدة والتحقق من نتيجته.

أُكمل تسجيل الخروج ثم تسجيل الدخول من جديد بحساب QA V2 بنجاح. عادت لوحة التحكم وهي تعرض صفراً من سجلات الديون، وهو تأكيد إضافي لاستمرار نظافة بيانات الحساب قبل طلب حذف هوية المصادقة النهائية.

بعد إعادة المصادقة الحديثة، أعاد طلب حذف Firebase Auth الحالة `200`. وتحقق طلب القراءة اللاحق من النتيجة وأعاد `USER_NOT_FOUND`، بما يؤكد أن حساب QA V2 لم يعد موجوداً في Firebase Auth. وبذلك اكتمل التنظيف: حُذف مرفق الاختبار من السجل، وحُذف كائنه الوحيد من Firebase Storage، وحُذف سجل الدين التجريبي، ثم حُذفت هوية حساب QA.

### حالة التسليم

حُفظت نقطة التسليم `d5bb110b` بعد اكتمال اختبارات الإنتاج والتنظيف. وتطابق التزام الفرع `main` في GitHub مع الالتزام المحلي الكامل `d5bb110bb28993e9612e9591faa787ce140dfd89`. وأعيد تشغيل حزمة ضمان الجودة النهائية بنجاح: **65 اختبار وحدة**، وفحص TypeScript، وبناء Vercel.
